...
# ============================================================================
# Xray VLESS/XHTTP/Reality Installer (Улучшенная версия)
...
check_root() {
  if [[ "$EUID" -ne 0 ]]; then
    print_error "Скрипт должен запускаться от имени root (используйте sudo)."
  fi
}

check_interactive() {
  if ! [[ -t 0 ]]; then
    print_error "Скрипт требует интерактивного терминала. Запустите его напрямую."
  fi
}

get_public_ip() {
  timeout 5 curl -4s https://icanhazip.com 2>/dev/null || hostname -I | awk '{print $1}' | cut -d' ' -f1
}
...
check_dns() {
  ...
  if ! host "$domain" | grep -q "$server_ip"; then
    print_warning "Домен $domain не разрешается в IP сервера ($server_ip). Проверьте DNS-записи."
    read -p "$(echo -e "${SOFT_YELLOW}⚠${RESET} Продолжить без проверки DNS? [Y/n]: ")" confirm < /dev/tty 2>/dev/null || confirm="Y"
    if [[ ! "$confirm" =~ ^[Yy]$ ]]; then
      print_error "Проверьте DNS-записи для домена $domain."
    fi
  fi
}
...
check_dependency() {
  ...
  if ! command -v "$dep" &>/dev/null; then
    print_info "Установка зависимости: $dep"
    apt-get install -y "$dep" >/dev/null 2>&1 || print_error "Не удалось установить $dep."
  fi
}
...
prompt_domain() {
  ...
  if [[ -n "$DOMAIN" ]]; then
    ...
    while true; do
      read -p "$(echo -e "${LIGHT_GRAY}Введите ваш домен${RESET} (например, wishnu.duckdns.org): ")" input_domain < /dev/tty 2>/dev/null || print_error "Требуется интерактивный ввод домена."
...
configure_fail2ban() {
  ...
  check_dependency "fail2ban-client"
...
configure_caddy() {
  ...
  if [[ -f "$CADDYFILE" ]]; then
    cp "$CADDYFILE" "${CADDYFILE}.backup-$(date +%Y%m%d-%H%M%S)" 2>/dev/null || print_warning "Не удалось создать резервную копию Caddyfile."
  fi
...
generate_xray_config() {
  ...
  if [[ -f "$XRAY_KEYS" ]]; then
    ...
    if [[ -z "$priv_key" || -z "$pub_key" ]]; then
      print_error "Не удалось сгенерировать ключи для Xray."
    fi
...
EOF
  ...
  if ! xray test -config "$XRAY_CONFIG" >/dev/null 2>&1; then
    print_error "Ошибка в конфигурации Xray. Проверьте файл: $XRAY_CONFIG"
  fi
